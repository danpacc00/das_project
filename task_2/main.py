...
def main():
    ...
    up_wall_start = np.array([-5, 5])
    up_wall_end = np.array([5, 5])

    down_wall_start = np.array([-5, -5])
    down_wall_end = np.array([5, -5])
    ...
    middle = (up_wall + down_wall) / 2

    # Define the targets at the right of the corridor
    targets = np.random.rand(args.nodes, 2) * 20 - 5
    initial_poses = np.random.rand(args.nodes, 2) * 20 - 5

    targets[:, 0] += up_wall_end[0] + 20
    initial_poses[:, 0] += up_wall_start[0] - 20
    ...
    if not args.no_plots:
        ...
        animation2(
            zz, np.linspace(0, kk, kk), nx.adjacency_matrix(graph).toarray(), targets, up_wall, down_wall, middle
        )
